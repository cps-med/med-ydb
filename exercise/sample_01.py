...
#   python3 /opt/med-ydb/exercise/sample_01.py'
# ---------------------------------------------------------------------------
...
def main():
    ...
    key3.value = bytes('你好世界!', encoding="utf-8")          # Value can be anything that can be encoded to `bytes`
    print(f" {key3}: {str(key3.value, encoding='utf-8')}\n")   # Returned values are `bytes` objects
    ...
    for subscript in key1.subscripts:
        ...
        print(f"{str(sub_key):>18}: {sub_key.value}")

    # Delete the value of '^hello', but not any of its child nodes
    key1.delete_node()

    print(f"{str(key1):>18}: {key1.value}")     # No value is printed
